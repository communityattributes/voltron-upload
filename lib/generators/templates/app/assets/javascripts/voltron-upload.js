  ...
  return {
    ...
    addUpload: function(){
      ...
      $.each(input.data('commit'), function(index, id){
        form.prepend($('<input />', { type: 'hidden', name: Voltron.getModule('Upload').getParamName(dz.input, 'commit'), value: id }));
      });
    ...
    onBeforeSend: function(file, xhr, data){
      ...
      var form = this.form;
      var commitName = Voltron.getModule('Upload').getParamName(this.input, 'commit');
    ...
    onSuccess: function(file, data){
      ...
      $.each(data.uploads, function(index, id){
        ...
        $(form).prepend($('<input />', { type: 'hidden', name: Voltron.getModule('Upload').getParamName(input, 'commit'), value: id }));
        $(form).find('input[name="' + Voltron.getModule('Upload').getParamName(input, 'remove') + '"]').remove();
      });
    },
    ...
    onRemove: function(file){
      ...
      var commitName = Voltron.getModule('Upload').getParamName(this.input, 'commit');
      var removeName = Voltron.getModule('Upload').getParamName(this.input, 'remove');
